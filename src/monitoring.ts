...
class UsageMonitor {
  ...
  track(action: keyof Omit<UsageMetrics, 'lastReset'>): void {
    ...
    if (this.metrics.lobbiesCreated % 10 === 0) {
  ...
  private checkThresholds(): void {
    for (const [metric, threshold] of Object.entries(this.ALERT_THRESHOLDS)) {
      const current = this.metrics[metric as keyof typeof this.ALERT_THRESHOLDS];
      if (current >= threshold) {
        console.warn(`🚨 USAGE ALERT: ${metric} has reached ${current} (threshold: ${threshold})`);
  ...
  getMetrics(): UsageMetrics & { timeRemaining: number } {
    ...
    return {
      ...this.metrics,
      timeRemaining: this.RESET_INTERVAL - (Date.now() - this.metrics.lastReset)
    };
  }
